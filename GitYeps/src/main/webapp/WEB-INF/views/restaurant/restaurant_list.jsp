...
<body>
	<div id="main">
		<div class="header">
			<div class="content-container" style="width: 1100px;height:100%;margin: 0 auto;padding: 0 15px;">
				<div class="clearfix layout-block layout-full search-page-top" style="margin: 0 -15px;">
					<div class="column column-alpha " style="float: left;padding: 0 15px;min-height: 1px;box-sizing: border-box;width: 100%;height:100%;">
		...
		<div class="container" style="width:1700px; height:2269px;">
			<div class="column-alpha" style="width:700px; height:100%;display:inline-block;float:left;padding:0px 15px 0px 15px">
				<ul>
					<c:forEach var="dto" items="${map.list}">
						<li style="width: 700px;height:190px;padding:16px 0px 16px 0px;border-bottom: 1px solid #e6e6e6;">
							<div class="biz-listing-large"style="width: 700px; height: 100px; position: relative;">
								...
								<div class="second-attribute"
									style="width: 280px; height: 90px; display: inline-block; position: absolute">
				...
				<div class="page">
					...
					<div class="paging" style="width:450px;height:40px;display:inline-block;float:left">
						<c:if test="${map.boardPager.curBlock > 1}">
                    <a class="number" href="javascript:list('1')">[처음]</a>
                </c:if>
                <!-- **이전페이지 블록으로 이동 : 현재 페이지 블럭이 1보다 크면 [이전]하이퍼링크를 화면에 출력 -->
                <c:if test="${map.boardPager.curBlock > 1}">
                    <a class="number" href="javascript:list('${map.boardPager.prevPage}')">[이전]</a>
                </c:if>
                ...
                <c:if test="${map.boardPager.curBlock <= map.boardPager.totBlock}">
                    <a class="number" href="javascript:list('${map.boardPager.nextPage}')">[다음]</a>
                </c:if>
                
                <!-- **끝페이지로 이동 : 현재 페이지가 전체 페이지보다 작거나 같으면 [끝]하이퍼링크를 화면에 출력 -->
                <c:if test="${map.boardPager.curPage <= map.boardPager.totPage}">
                    <a class="number" href="javascript:list('${map.boardPager.totPage}')">[끝]</a>
                </c:if>
					</div>
				</div>
			</div>
			<div class="column-beta" style="width:330px;height: 300px;float:left;padding:0px 15px 0px 15px;box-sizing: border-box;">
				<div class="wrap">
		...
		for (var i = 0; i < roadAddrPart1.length; i++) {
			naver.maps.Service.geocode({address : roadAddrPart1[i]},
							function(status, response) {
								...
								iwContent = [
									'<div class="main" style="width:300px;height:140px;padding:12px;">',
...
//     	  $(".wrap").css({"position": "absolute","z-index": "auto"});
      } 
      else {
