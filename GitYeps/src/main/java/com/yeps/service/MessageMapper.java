...
public class MessageMapper {
	...
	public List<MessageDTO> messageList(int start, int end, String lMode, String name) {
		...
		if (lMode.equals("allList")) {
			...
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' order by msgNum desc)A)" + " where  rn between " + start + " and " + end;
		} else if (lMode.equals("allLocker")) {
			// 로그인한 회원의 보관함에 있는 쪽지를 가져온다.
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' and islocker=1 order by msgNum desc)A)" + " where rn between " + start + " and " + end;
			...
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where sender='" + name
					+ "' order by msgNum desc)A) " + "where rn between " + start + " and " + end;
		} else if (lMode.equals("readMsg")) {
			// 읽은 쪽지중 쪽지함에 있는 쪽지
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' and readNum=1 and islocker=0 order by msgNum desc)A)" + " where rn between " + start + " and "
					+ end;
		} else if (lMode.equals("readLocker")) {
			// 읽은 쪽지중 보관함에 있는 쪽지
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' and readNum=1 and islocker=1 order by msgNum desc)A)" + " where rn between " + start + " and "
					+ end;
		} else if (lMode.equals("noneMsg")) {
			// 안읽은 쪽지함 쪽지
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' and readNum=0 and islocker=0 order by msgNum desc)A)" + " where rn between " + start + " and "
					+ end;
		} else if (lMode.equals("noneLocker")) {
			// 안읽은 보관함 쪽지
			sql = "select * from (select rownum rn, A.* from (select * from yeps_message where receiver='" + name
					+ "' and readNum=0 and islocker=1 order by msgNum desc)A) " + "where rn between " + start + " and "
					+ end;
		}
