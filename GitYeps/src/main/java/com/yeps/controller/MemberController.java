...
import com.yeps.service.MemberPager;
...
public class MemberController {
	...
	public ModelAndView listMember(HttpServletRequest req, HttpSession session, @RequestParam(defaultValue="1") int curPage) {
	...
	public ModelAndView joinMemberPro(HttpServletRequest req, @ModelAttribute MemberDTO dto, HttpSession session, BindingResult result){
		...
		if(isExistJumin) {
			...
			if (res>0){
				...
				MemberPhotoDTO mainPhoto = memberPhotoMapper.getMemberMainPhoto(dto2.getMnum());
	...
	public HashMap<String, String> loginMemberAjaxPro(HttpServletRequest req, @ModelAttribute MemberDTO dto, HttpSession session) {
		...
		if (dto.getEmail() == null || dto.getPasswd()==null || dto.getEmail().trim().equals("") || dto.getPasswd().trim().equals("")){
	...
	public ModelAndView profileMemberPro(HttpServletRequest req, @ModelAttribute MemberDTO dto2, HttpSession session, BindingResult result) {
	...
	public ModelAndView deletePhoto_Member(HttpServletRequest req, HttpSession session) {
		...
		MemberDTO userDTO = memberMapper.getMemberByMnum(Integer.parseInt(mnum));	// 변경한 사진을 가진 유저의 정보
		if(memberDTO.getMnum() == Integer.parseInt(mnum) || memberDTO.getIsmanager().equals("y")) {
			int res = memberPhotoMapper.deleteMemberPhoto(Integer.parseInt(filenum), Integer.parseInt(mnum), ismainphoto);
			if(res >0) {
